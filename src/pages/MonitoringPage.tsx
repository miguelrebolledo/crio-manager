    ...
    <Layout>
      <div style={{ padding: '24px 28px' }}>
        ...
        <div style={{ marginBottom: 18 }}>
          <h1 style={{ fontSize: 17, fontWeight: 500, color: '#3D3D3A', margin: 0 }}>Monitoreo</h1>
          <div style={{ fontSize: 12, color: '#9C9A92', marginTop: 3 }}>
            {loading ? 'Cargando...' : `${total} visita${total !== 1 ? 's' : ''} — todos los proyectos`}
          </div>
        </div>
        ...
        {critF > 0 && (
          <div style={{ background: '#FCEBEB', border: '0.5px solid #F7C1C1', borderRadius: 9, padding: '10px 14px', fontSize: 12, color: '#791F1F', marginBottom: 14, display: 'flex', gap: 8 }}>
            ...
            <div><strong>{critF} hallazgo{critF > 1 ? 's' : ''} crítico{critF > 1 ? 's' : ''} sin respuesta</strong> — requieren acción inmediata de la coordinadora.</div>
          </div>
        )}
            ...
            <div style={{ background: '#fff', border: '0.5px solid #E8E6DE', borderRadius: 10, overflow: 'hidden' }}>
              {loading ? (
                ...
                <div style={{ padding: 48, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 14, color: '#9C9A92' }}>Sin visitas registradas</div>
                </div>
              ) : visits.map((v, i) => {
                ...
                return (
                  <div key={v.id} style={{ borderBottom: i < visits.length - 1 ? '0.5px solid #E8E6DE' : 'none' }}>
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4, flexWrap: 'wrap' }}>
                          ...
                          <span style={{ fontSize: 11, background: '#F1EFE8', color: '#444441', padding: '2px 8px', borderRadius: 20, fontWeight: 500 }}>
                            ...
                            <div key={f.id} style={{ marginBottom: 8, background: '#fff', border: '0.5px solid #E8E6DE', borderRadius: 8, padding: '10px 13px' }}>
                              ...
                              {f.response_text && (
                                <div style={{ marginTop: 8, fontSize: 12, color: '#73726C', background: '#F8F7F4', borderRadius: 6, padding: '7px 10px' }}>
                                  <i className="ti ti-message-reply" style={{ fontSize: 12, marginRight: 4 }} />
                                  {f.response_text}
                                </div>
                              )}
                            </div>
                    ...
                    {isExp && v.findings.length === 0 && (
                      <div style={{ background: '#F8F7F4', borderTop: '0.5px solid #E8E6DE', padding: '12px 16px', fontSize: 13, color: '#9C9A92', textAlign: 'center' }}>
                        Sin hallazgos en esta visita.
                      </div>
                    )}
                  </div>
                )
