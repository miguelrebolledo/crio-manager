// src/components/project/TabMonitoring.tsx
...
function FindingRow({ finding, onUpdate }: { finding: Finding; onUpdate: () => void }) {
  ...
  const isMonitor     = ['EXTERNAL_MONITOR','ADMIN','PM_CRIO'].includes(role)
  ...
  const canDecide  = isMonitor && finding.status === 'RESPONDED'
    ...
    <div style={{ borderBottom: '0.5px solid #E8E6DE' }}>
      ...
      >
        ...
        <i className={`ti ti-chevron-${expanded ? 'up' : 'down'}`}
          style={{ fontSize: 15, color: '#9C9A92', flexShrink: 0, marginTop: 2 }} />
      </div>
      ...
      {expanded && (
        <div style={{ background: '#F8F7F4', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ background: '#fff', border: '0.5px solid #E8E6DE', borderRadius: 9, padding: 14 }}>
              {finding.status === 'REJECTED' && (
                <div style={{ background: '#FAEEDA', border: '0.5px solid #FAC775', borderRadius: 7, padding: '8px 11px', fontSize: 12, color: '#633806', marginBottom: 10 }}>
                  ...
                  El monitor rechazó tu respuesta anterior. Incluye evidencia adicional más detallada.
                </div>
              )}
              <textarea
                style={inp}
          ...
          {canDecide && (
            <div style={{ background: '#fff', border: '0.5px solid #E8E6DE', borderRadius: 9, padding: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 500, color: '#3D3D3A', marginBottom: 8 }}>Decisión del monitor</div>
              <textarea
                style={inp}
                value={decisionText}
...
function VisitModal({
  ...
  useEffect(() => {
    supabase.from('users')
      ...
      .in('role', ['EXTERNAL_MONITOR','ADMIN','PM_CRIO'])
      .eq('is_active', true)
      .order('full_name')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.monitor_id || !form.scheduled_date) { setError('Monitor y fecha son obligatorios.'); return }
    setSaving(true)
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 13 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div>
                <label style={{ fontSize: 11, color: '#9C9A92', fontWeight: 500, display: 'block', marginBottom: 4 }}>Fecha agendada *</label>
                <input style={inp} type="date" value={form.scheduled_date}
                  onChange={e => setForm(f => ({ ...f, scheduled_date: e.target.value }))} required />
              </div>
            </div>
            <div>
              <label style={{ fontSize: 11, color: '#9C9A92', fontWeight: 500, display: 'block', marginBottom: 4 }}>Monitor asignado *</label>
              <select style={inp} value={form.monitor_id} onChange={e => setForm(f => ({ ...f, monitor_id: e.target.value }))} required>
                ...
                {monitors.map(m => <option key={m.id} value={m.id}>{m.full_name} — {m.email}</option>)}
              </select>
            </div>
            <div>
          ...
          <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 13 }}>
            ...
            <div>
              ...
              <textarea style={{ ...inp, minHeight: 80, resize: 'vertical' }}
                ...
                placeholder="Describe el hallazgo detectado con suficiente detalle para que la coordinadora pueda responder..." required />
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '14px 20px', borderTop: '0.5px solid #E8E6DE' }}>
            <button type="button" onClick={onClose} style={{ background: 'transparent', border: '0.5px solid #D3D1C7', color: '#73726C', padding: '7px 16px', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>Cancelar</button>
...
function VisitCard({ visit, onUpdate }: { visit: Visit; onUpdate: () => void }) {
  ...
  const role = user?.role ?? ''
  const isMonitor = ['EXTERNAL_MONITOR','ADMIN','PM_CRIO'].includes(role)
  const canAddFinding = isMonitor && visit.status === 'COMPLETED'
  const canEdit = ['ADMIN','PM_CRIO','EXTERNAL_MONITOR'].includes(role)
  ...
  return (
    <div style={{ background: '#fff', border: '0.5px solid #E8E6DE', borderRadius: 10, overflow: 'hidden', marginBottom: 10 }}>
      ...
      <div
        ...
        onMouseLeave={e => (e.currentTarget.style.background = '')}
      >
        <div style={{ width: 38, height: 38, borderRadius: 9, background: vt.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <i className="ti ti-calendar-event" style={{ fontSize: 17, color: vt.color }} />
        </div>

        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4, flexWrap: 'wrap' }}>
            <span style={{ ...vt, fontSize: 11, padding: '2px 8px', borderRadius: 20, fontWeight: 500 }}>
              Visita de {VISIT_TYPE_LABELS[visit.visit_type]}
            </span>
        ...
        <div style={{ borderTop: '0.5px solid #E8E6DE' }}>
          ...
          {visit.findings.length === 0 ? (
            <div style={{ padding: '16px 20px', fontSize: 13, color: '#9C9A92', textAlign: 'center' }}>
              Sin hallazgos registrados en esta visita.
            </div>
          ) : (
            visit.findings.map(f => (
              <FindingRow key={f.id} finding={f} onUpdate={onUpdate} />
            ))
          )}
          ...
          {canAddFinding && (
            <div style={{ padding: '10px 16px', borderTop: visit.findings.length > 0 ? '0.5px solid #E8E6DE' : 'none' }}>
              <button onClick={() => setShowFinding(true)} style={{ background: '#FCEBEB', color: '#791F1F', border: '0.5px solid #F7C1C1', padding: '5px 12px', borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                <i className="ti ti-plus" style={{ fontSize: 13 }} /> Registrar hallazgo
              </button>
            </div>
          )}
        </div>
...
// ── MAIN TAB ─────────────────────────────────────────────────
export default function TabMonitoring({ projectId }: { projectId: string }) {
  const { user } = useAuth()
  const [visits, setVisits]     = useState<Visit[]>([])
  const [loading, setLoading]   = useState(true)
  const [showModal, setShowModal] = useState(false)

  const canSchedule = ['ADMIN','PM_CRIO','EXTERNAL_MONITOR'].includes(user?.role ?? '')
  ...
  const criticalF = allFindings.filter(f => f.category === 'CRITICAL' && f.status === 'OPEN').length
    ...
    <div>
      ...
      {criticalF > 0 && (
        <div style={{ background: '#FCEBEB', border: '0.5px solid #F7C1C1', borderRadius: 9, padding: '10px 14px', fontSize: 12, color: '#791F1F', marginBottom: 14, display: 'flex', gap: 8 }}>
          ...
          <div><strong>{criticalF} hallazgo{criticalF > 1 ? 's' : ''} crítico{criticalF > 1 ? 's' : ''} sin respuesta.</strong> Requieren acción inmediata de la coordinadora.</div>
        </div>
      )}
      ...
      {/* visits */}
      {loading ? (
        <div style={{ padding: 32, textAlign: 'center', fontSize: 13, color: '#9C9A92' }}>Cargando visitas...</div>
      ) : visits.length === 0 ? (
